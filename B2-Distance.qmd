...
In this chapter, we introduce **support-vector machines (SVMs)** and other distance-based methods. **Important:** distance-based models need scaled features!

## K-Nearest-Neighbor

**K-nearest-neighbor (kNN)** is a simple algorithm that stores all the available cases and classifies new data based on a similarity measure. It classifies a data point according to how its $k$ nearest neighbors are classified.

Let us first see an example:

```{r chunk_chapter4_32, fig.cap="The blue point is the one we want to classify; its class is decided from the classes of its nearest neighbours."}
x = scale(iris[,1:4])
y = iris[,5]
plot(x[-100,1], x[-100, 3], col = y,
     xlab = "Sepal length (scaled)", ylab = "Petal length (scaled)")
...
```{r chunk_chapter4_36, fig.cap="SVM predictions (left) vs. observed species (right) on the test set. Mismatched colours mark misclassified points."}
...
We see that there are some discrepancies. Confusion matrix:
...
Ordination is used in exploratory analysis and, compared to clustering, orders similar objects together, so there is a relationship between clustering and ordination. Here is a **PCA** ordination of the iris dataset:

```{r chunk_chapter3_24, fig.cap="PCA biplot of the iris traits: points are observations, arrows are the original traits projected onto the first two principal components."}
...
`r hide("Show solution — hierarchical clustering")`
...
`r hide("Show solution — density-based clustering")`
...
`r hide("Show solution — model-based clustering")`
...
`r hide("Show solution — ordination")`
