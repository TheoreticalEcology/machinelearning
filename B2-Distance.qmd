...
tuning_results =
    sapply(1:length(hyper_kernel), function(k) {
        ...
        for(j in 1:cv) {
          ...
          predictions = kknn(as.factor(survived)~., train = train_inner, test = test_inner, k = hyper_k[k], scale = FALSE, kernel = hyper_kernel[k])
...
tuning_results = 
      ...
      for(i in 1:cv) {
        ...
        tmp_cv = Metrics::auc(test_outer$survived, predictions$prob[,2])
       }
