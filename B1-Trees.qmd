...
```{r chunk_chapter4_23, fig.cap="A single regression tree for Ozone. Each split shows the rule and each leaf the predicted value."}
...
```{r chunk_chapter4_24, fig.cap="Regression-tree predictions (red) against temperature. The step-like shape is characteristic of trees."}
...
```{r chunk_chapter4_25, message=FALSE, warning=FALSE, fig.cap="Random-forest predictions (red) against temperature. Averaging many trees gives a smoother fit than a single tree."}
...
```{r chunk_chapter4_29__BRT2, cache=TRUE, fig.cap="Boosted-tree predictions using 1, 2, 3, and 4 trees (panel titles). With more sequential trees the fit gradually improves."}
...
::: exercise
#### Understanding complexity in Regression Trees
...
Quick check — a very small `mincut` produces a tree that: `r webexercises::mcq(c(answer = "fits the training data closely but risks overfitting (high variance)", "is strongly regularized and underfits", "cannot be fitted at all"))`
...
::: exercise
#### Understanding complexity in Random forest
...
Quick check — increasing `nodesize` makes the random forest: `r webexercises::mcq(c(answer = "simpler (more bias, less variance)", "more complex (less bias, more variance)", "unaffected"))`
...
::: exercise
#### Understanding complexity in Boosted regression trees
...
```{r chunk_chapter4_task_26, results='hide', message=FALSE, warning=FALSE, eval=FALSE, purl=FALSE}
...
Quick check — with a small `max_depth`, reaching a smooth fit requires: `r webexercises::mcq(c(answer = "more boosting rounds (trees)", "fewer boosting rounds", "a smaller learning rate only"))`
...
:::: exercise
#### Hyperparameter tuning of boosted regression trees
...
Quick check — we select hyperparameters by the cross-validated AUC rather than the training AUC because: `r webexercises::mcq(c(answer = "the training AUC keeps rising with complexity and would always pick the most complex model", "the training AUC cannot be computed for xgboost", "AUC is only defined out-of-sample"))`

::: {.callout-tip collapse="true" appearance="minimal"}
## Code template

```{r chunk_chapter4_brt_template, eval=FALSE}
library(xgboost)
set.seed(42)
...
```{r chunk_chapter4_brt_predict, results='hide', warning=FALSE, message=FALSE}
...
::: exercise
#### Bonus: Walk through a from-scratch BRT
...
`r hide("Show the implementation")`

```{r chunk_chapter4_naivebrt_helpers, echo = TRUE}
library(tree)
...
```{r chunk_chapter4_naivebrt_demo}
...
Quick check — a larger minimum node size in the random forest tends to: `r webexercises::mcq(c(answer = "reduce variance by growing simpler trees", "reduce bias by growing deeper trees", "have no effect on complexity"))`

::: {.callout-tip collapse="true" appearance="minimal"}
## Code template

```{r chunk_chapter4_rf_template, eval=FALSE}
library(randomForest)
set.seed(42)
data_obs = data_sub[!is.na(data_sub$survived),] 
data_obs$survived = as.factor(data_obs$survived)
...
```{r chunk_chapter4_rf_predict, results='hide', warning=FALSE, message=FALSE}
