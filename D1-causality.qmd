...
Methods for **causal inference** depend on whether we have dynamic or static data. The latter is the more common case. With static data, the central problem is **confounding**: if you have several correlated predictors, you can get spurious correlations between a given predictor and the response even when there is no causal effect.
...
```{r chunk_chapter6_12, eval=TRUE, fig.cap="Random-forest variable importance for two strongly collinear predictors, only one of which (x1) is causal. The importance is split nearly evenly between them."}
set.seed(123)
...
Variable importance is now split nearly evenly between `x1` and `x2`, even though only `x1` is causal.
...
When working with dynamic data, we can use an additional piece of information: the cause usually precedes the effect, which means we can test for a time-lag between cause and effect to determine the direction of causality. This way of testing is known as **Granger causality**. Here is an example:
