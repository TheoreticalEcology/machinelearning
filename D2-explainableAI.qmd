...
The cito package has quite extensive xAI functionalities. However, ranger, like most other machine learning packages, has no extensive xAI functionalities. Thus, to do xAI with ranger, we have to use a generic xAI package that can handle almost all machine learning models.
...
bio9 (Mean Temperature of Driest Quarter) is the most important variable.
...
One disadvantage of partial dependencies is that they are sensitive to correlated predictors. Accumulated local effects can be used to account for correlation between predictors.
...
data_sub = df[sample.int(nrow(df), 500),]

```

cito includes several xAI methods directly out of the box
...
<!-- ::: exercise -->
<!-- #### Interpret a fitted model (Titanic) -->
