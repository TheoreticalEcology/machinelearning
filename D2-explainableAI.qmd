...
```{r}
dnn = model = dnn(Presence~., data = data, batchsize = 100L, 
                  validation = 0.3, loss = "binomial", verbose = FALSE)
```
...
Moreover, try to refit the modell with the option bootstrap = 5. This may take a short while. Observe how the xAI options change. 
:::
