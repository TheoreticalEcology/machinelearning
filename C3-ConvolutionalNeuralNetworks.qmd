...
```{r chunk_c3_mnist_cito_plot}
plot(model.cito)
```

Evaluation:

```{r chunk_c3_mnist_cito_eval}
...
```{r chunk_c3_cifar_torch_train, eval=FALSE}
model_torch = net()
opt = optim_adam(params = model_torch$parameters, lr = 0.01)
...
```{r chunk_c3_cifar_cito_plot}
plot(model.cito)
```

Explainable AI:

```{r chunk_c3_cifar_cito_xai}
library(torch)
observation = 19
...
```{r chunk_c3_flower_torch_predict}
...
model |> 
  fit(x = train, y = keras3::to_categorical(labels, 5L))
```

Predictions:

```{r chunk_c3_flower_keras_predict}
# Prediction on training data:
...
```{r chunk_c3_flower_cito_submission}
write.csv(data.frame(y = pred), file = "cnn.csv")
```
...
model |> 
  fit(x = train, y = labels)
```

Predictions:

```{r chunk_c3_sdm_keras_predict}
# Prediction on training data:
...
```{r chunk_c3_sdm_cito_submission}
...
```{r chunk_c3_transfer_cito, eval=FALSE}
train_x = array(train$x/255, c(dim(train$x)))
test_x = array(test$x/255, c(dim(test$x)))
train_y =train$y
test_y = test$y
...
```{r chunk_c3_transfer_flower_cito, eval=FALSE}
data = EcoData::dataset_flower()
train = data$train/255
...
```{r chunk_c3_transfer_flower_torch_model}
library(keras3)
library(tensorflow)
...
```{r chunk_c3_transfer_flower_torch_data}
# dataloaders
...
```{r chunk_c3_transfer_flower_torch_train}
model = net()
...
```{r chunk_c3_transfer_flower_torch_plot}
...
```{r chunk_c3_mmn_cito_submission}
