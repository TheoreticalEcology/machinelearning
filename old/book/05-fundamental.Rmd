...
cat("Torch LBFGS weights:\n",as.numeric(interceptTorch), as.numeric(wTorch))
```
...
TensorFlow and thus Keras also allow use using LASSO and Ridge on the weights. Lets see what happens when we put an $L1$ (LASSO) regularization on the weights:
...
```{r chunk_chapter4_task_1, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
...
```{r chunk_chapter4_task_1_torch, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
...
Famous machine learning algorithms such as *Random Forest* and *Gradient Boosted trees* are based on classification and regression trees. **Hint**: Tree-based algorithms are not distance based and thus do not need scaling.
...
<br> \* <a href="https://www.youtube.com/watch?v=g9c66TUylZ4" target="_blank" rel="noopener">Regression trees</a> <br>
...
<br> \* <a href="https://www.youtube.com/watch?v=J4Wdy0Wc_xQ" target="_blank" rel="noopener">Random forests</a> <br>
...
<br> \* <a href="https://www.youtube.com/watch?v=D0efHEJsfHo" target="_blank" rel="noopener">Pruning trees</a> <br>
...
<br> After watching these videos, you should know what the different hyperparameters are doing and how to prevent trees / forests from doing something you don't want.
...
```{r chunk_chapter4_task_29, message=FALSE, warning=FALSE, echo=FALSE, purl=FALSE, include=TRUE}
...
```
We implemented a simplified boosted regression tree using R just for fun. Go through the code line by line and try to understand it. Ask, if you have any questions you cannot solve.
...
```{=html}
  <details>
    ...
    <p>
```
```{r chunk_chapter4_task_37, include=TRUE}
knn = kknn(class~., train = train, test = test, scale = FALSE,
           kernel = "optimal")
predKNN = predict(knn, newdata = test)
...
```{r chunk_chapter4_task_3, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter4_task_4, message=FALSE, warning=FALSE, echo=FALSE, include=TRUE}
cat("Linear:     ", round(coef(linear), 3))
cat("L1:         ", round(c(unlist(l1)[length(unlist(l1))],
                            unlist(l1)[1:(length(unlist(l1)) - 1 )]), 3))
...
```{r chunk_chapter4_task_3_torch, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
torch_manual_seed(321L)
set.seed(123)
...
```{r chunk_chapter4_task_3_torch_msg, message=FALSE, warning=FALSE, echo=FALSE, include=TRUE}
cat("Linear:     ", round(coef(linear), 3))
cat("L1:         ", round(c(as.numeric( l1_torch$`0.bias` ),as.numeric( l1_torch$`0.weight` )), 3))
...
Weights 4 and 5 are strongly pushed towards zero with $L1$ regularization, but $L2$ regularization shrinks more in general. *Note*: The weights are not similar to the linear model! (But often, they keep their sign.)
...
```{r chunk_chapter4_task_5, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter4_task_6, message=FALSE, warning=FALSE, echo=FALSE, include=TRUE}
cat("Linear:     ", round(coef(linear), 3))
cat("L1:         ", round(c(unlist(l1)[length(unlist(l1))],
                            unlist(l1)[1:(length(unlist(l1)) - 1 )]), 3))
...
```{r chunk_chapter4_task_7, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter4_task_8, message=FALSE, warning=FALSE, echo=FALSE, include=TRUE}
cat("Linear:     ", round(coef(linear), 3))
cat("L2:         ", round(c(unlist(l2)[length(unlist(l2))],
                            unlist(l2)[1:(length(unlist(l2)) - 1 )]), 3))
...
```{r chunk_chapter4_task_5_torch, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
torch_manual_seed(321L)
set.seed(123)
...
```{r chunk_chapter4_task_5_torch_msg, message=FALSE, warning=FALSE, echo=FALSE, include=TRUE}
cat("Linear:     ", round(coef(linear), 3))
cat("L1:         ", round(c(as.numeric( l1_torch$`0.bias` ),as.numeric( l1_torch$`0.weight` )), 3))
cat("L1 lesser:         ", round(c(as.numeric( l1_torch_lesser$`0.bias` ),as.numeric( l1_torch_lesser$`0.weight` )), 3))
```

```{r chunk_chapter4_task_6_torch, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
torch_manual_seed(321L)
set.seed(123)
...
```{r chunk_chapter4_task_6_torch_msg, message=FALSE, warning=FALSE, echo=FALSE, include=TRUE}
cat("Linear:     ", round(coef(linear), 3))
cat("L2:         ", round(c(as.numeric( l2_torch$`0.bias` ),as.numeric( l2_torch$`0.weight` )), 3))
...
```{r chunk_chapter4_task_9, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter4_task_10, message=FALSE, warning=FALSE, echo=FALSE, include=TRUE}
cat("Linear:     ", round(coef(linear), 3))
cat("L1:         ", round(c(unlist(l1)[length(unlist(l1))],
                            unlist(l1)[1:(length(unlist(l1)) - 1 )]), 3))
cat("L1, lesser: ", round(c(unlist(l1_lesser)[length(unlist(l1_lesser))],
...
```{r chunk_chapter4_task_11, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter4_task_12, message=FALSE, warning=FALSE, echo=FALSE, include=TRUE}
cat("Linear:     ", round(coef(linear), 3))
cat("L2:         ", round(c(unlist(l2)[length(unlist(l2))],
                            unlist(l2)[1:(length(unlist(l2)) - 1 )]), 3))
cat("L2, lesser: ", round(c(unlist(l2_lesser)[length(unlist(l2_lesser))],
...
```{r chunk_chapter4_task_10_torch, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
torch_manual_seed(321L)
set.seed(123)
...
```{r chunk_chapter4_task_10_torch_msg, message=FALSE, warning=FALSE, echo=FALSE, include=TRUE}
cat("Linear:     ", round(coef(linear), 3))
cat("L1:         ", round(c(as.numeric( l1_torch$`0.bias` ),as.numeric( l1_torch$`0.weight` )), 3))
cat("L1 lesser:         ", round(c(as.numeric( l1_torch_lesser$`0.bias` ),as.numeric( l1_torch_lesser$`0.weight` )), 3))
...
```{r chunk_chapter4_task_11_torch, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
torch_manual_seed(321L)
set.seed(123)
...
```{r chunk_chapter4_task_11_torch_msg, message=FALSE, warning=FALSE, echo=FALSE, include=TRUE}
cat("Linear:     ", round(coef(linear), 3))
cat("L2:         ", round(c(as.numeric( l2_torch$`0.bias` ),as.numeric( l2_torch$`0.weight` )), 3))
cat("L2 lesser:         ", round(c(as.numeric( l2_torch_lesser$`0.bias` ),as.numeric( l2_torch_lesser$`0.weight` )), 3))
...
For pushing weights towards zero, $L1$ regularization is used rather than $L2$. Higher regularization leads to smaller parameters (maybe in combination with smaller learning rates).
...
```{r chunk_chapter4_task_13, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter4_task_13_torch, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
torch_manual_seed(321L)
set.seed(123)
...
```{r chunk_chapter4_task_15, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter4_task_141_torch, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
torch_manual_seed(321L)
set.seed(123)
...
```{r chunk_chapter4_task_15_cito, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
# devtools::install_github("citoverse/cito")
library(cito)
...
```{r chunk_chapter4_task_16, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter4_task_142_torch, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
torch_manual_seed(321L)
set.seed(123)
...
```{r chunk_chapter4_task_16_cito, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
# devtools::install_github("citoverse/cito")
library(cito)
...
```{r chunk_chapter4_task_17, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter4_task_143_torch, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
torch_manual_seed(321L)
set.seed(123)
...
```{r chunk_chapter4_task_17_cito, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
# devtools::install_github("citoverse/cito")
library(cito)
...
```{r chunk_chapter4_task_18, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter4_task_144_torch, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
torch_manual_seed(321L)
set.seed(123)
...
```{r chunk_chapter4_task_18_cito, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
# devtools::install_github("citoverse/cito")
library(cito)
...
```{r chunk_chapter4_task_19, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter4_task_145_torch, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
torch_manual_seed(321L)
set.seed(123)
...
```{r chunk_chapter4_task_19_cito, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
# devtools::install_github("citoverse/cito")
library(cito)
