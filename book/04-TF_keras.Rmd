...
```{r chunk_chapter3_task_torch_7, eval=TRUE}
library(torch)
...
```{r chunk_chapter3_task_torch_12, eval=TRUE}
do_something_torch = function(x = matrix(0.0, 10L, 10L)){
...
```{r chunk_chapter3_task_torch_13, eval=TRUE}
test = matrix(0.0, 100L, 100L)
microbenchmark::microbenchmark(do_something_R(test), do_something_torch(test))
...
```{r chunk_chapter3_task_torch_14, eval=TRUE}
test = matrix(0.0, 1000L, 500L)
testTorch = torch_tensor(test)
...
```{r chunk_chapter3_task_torch_16, eval=TRUE}
library(torch)
...
```{r chunk_chapter3_task_torch_25, eval=TRUE}
library(torch)
torch::torch_manual_seed(42L)
...
```{r chunk_chapter3_task_36_torch, eval=FALSE, purl=FALSE}
library(torch)
...
```{r chunk_chapter3_task_37_torch, eval=FALSE, purl=FALSE}
optimizer_torch = optim_adam(params = model_torch$parameters, lr = 0.001)
```
...
```{r chunk_chapter3_task_38_torch, eval=FALSE, purl=FALSE}
# Calculate number of training steps.
epochs = 100
batch_size = 32
steps = round(nrow(x)/batch_size*epochs)
...
```{r chunk_chapter3_task_381_torch, eval=FALSE, purl=FALSE}
plot(y = log_losses, x = 1:steps, xlab = "Epoch", ylab = "MSE")
```

**7. Create predictions.**

```{r chunk_chapter3_task_382_torch, eval=FALSE, purl=FALSE}
...
```{r chunk_chapter3_task_39_torch, eval=FALSE, purl=FALSE}
fit = lm(Ozone ~ ., data = data)
pred_lm = predict(fit, data)
rmse_lm = mean(sqrt((y - pred_lm)^2))
rmse_torch = mean(sqrt((y - pred_torch)^2))
...
```{r chunk_chapter3_task_41_torch, eval=TRUE, message=FALSE, warning=FALSE}
library(torch)
...
```{r chunk_chapter3_task_43_torch, eval=TRUE, message=FALSE, warning=FALSE}
library(torch)
...
```{r chunk_chapter3_task_442_torch, eval=TRUE, message=FALSE, warning=FALSE}
library(torch)
...
```{r chunk_chapter3_task_47_torch, eval=TRUE}
library(torch)
