...
The idea of lasso and ridge regularization is to put some type of rubber band on the weights and pull them to zero if the data does not strongly push them away from zero.Having this in mind it is claer that important weights, which are supported by the data, are able be different to zero.
...
In this lecture we will explore regression and classifaction trees at the example of the airquality data set. We first prepare and visualize the data and afterwards fit a decision tree. 
...
With the forcats package we can easily handle and mutate factor variables.
...
We will collapse titles with low occurrences into one title:
...
20% NAs!
Either we remove all observations with NAs, or we impute (fill) the missing values, e.g. with the median age:
...
However, age itself might depend on other variables such as sex, class and title. We want to fill the NAs with the median age of these groups.
...
### Pre-processing and feature selection
Keras cannot handle factors and we have to scale the data.

For now we will sub-select a batch of predictors and:

- scale the numeric predictors
- change the factors with only two groups/levels into integer
```{r}
data_sub =
  data %>%
...
One hot encoding of factors with > 2 levels:
```{r}
...
```
Add the dummy encoded variables to the dataset:
...
### Model fitting
In the next step we will fit a keras model on the train split of the inner split:
```{r}
model = keras_model_sequential()
model %>%
...
### Model evaluation
We will predict survived for the test set of the inner split and calculate the accuracy:
...
When we are satisfied with the performance of our model in the inner split, we will create predictions for the test split of the outer split:

select all observations that belong to the outer test split (the filter function)
remove the subset and survived (NAs) columns
```{r}
submit= 
  data_sub %>% 
    filter(subset == "test") %>% 
...
http://rhsbio6.uni-regensburg.de:8500
To do so, we have to transform our survived probablities into actual 0/1 predictions (probablities are not allowed) and create a csv:
