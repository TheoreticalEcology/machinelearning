...
```{r chunk_chapter2_0, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
cat(
...
```{r chunk_chapter2_11, cache=TRUE}
library(tensorflow)
library(keras)
...
```{r chunk_chapter2_13, cache=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	#Already sets R's random seed.
...
```{r chunk_chapter2_15, results='hide', message=FALSE, warning=FALSE}
library(rpart)
```
...
```{r chunk_chapter2_21, cache=TRUE, results='hide', message=FALSE, warning=FALSE}
library(xgboost)
set.seed(123)
```

```{r chunk_chapter2_22__BRT1, cache=TRUE}
...
```{r chunk_chapter2_23__BRT2, cache=TRUE}
...
```{r chunk_chapter2_24__BRT3, cache=TRUE}
...
```{r chunk_chapter2_25__BRT4, cache=TRUE}
set.seed(123)
...
```{r chunk_chapter2_31, eval=FALSE}
...
```{r chunk_chapter2_32, echo=FALSE, purl=FALSE}
...
```{r chunk_chapter2_36, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
cat(
  '<iframe width="560" height="315" 
...
```{r chunk_chapter2_37, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
cat(
  '<iframe width="560" height="315" 
...
```{r chunk_chapter2_38, message=FALSE}
library(keras)
library(tensorflow)
library(tidyverse)
```
...
```{r chunk_chapter2_61, eval=FALSE}
...
```{r chunk_chapter2_62, message=FALSE}
library(EcoData)
library(tidyverse)
library(mlr3)
...
```{r chunk_chapter2_67, eval=FALSE}
...
```{r chunk_chapter2_68 mlr2, eval=FALSE}
preds = 
...
```{r chunk_chapter2_72, eval=FALSE}
set.seed(123)
...
```{r chunk_chapter2_73, eval=FALSE}
preds = 
...
```{r chunk_chapter2_78, eval=FALSE}
set.seed(123)
...
```{r chunk_chapter2_79, eval=FALSE}
preds = 
  sapply(1:3, function(i) result$learners[[i]]$predict(transformed_task, 
                                                        row_ids = (1:nrow(data))[is.na(data$Hazardous)])$data$prob[, "1", drop = FALSE])
dim(preds)
