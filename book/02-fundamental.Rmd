...
We assume that the residuals are normally distributed, and our loss function to be the mean squared error: $\text{mean}(predicted~ozone - true~ozone)^{2}$
...
LASSO $\left(penalty \propto \sum_{}^{} \text{abs}(weights) \right)$ and ridge $\left(penalty \propto \sum_{}^{} weights^{2} \right)$ have slightly different properties. They are best understood if we express those as the effective prior preference they create on the parameters:
