...
for(i in 1:10) {
  ...
  model %>%
  compile(loss = loss_binary_crossentropy, optimizer = optimizer_adamax(0.0072))
...
Again, this sounds very complicated but it is again quite easy with Keras.
...
model = keras::keras_model(
  ...
  outputs = layer_flatten(layer_dense(densenet$output,
                                      units = 10L,
                                      activation = "softmax"
                                      )
                          )
...
If you are more interested in this topic (you should), read this 
...
getData = function(oversample = TRUE, undersample = FALSE){
  ...
  if(undersample){
    ...
    train <<- trainLocal[newIndices,,,]
    labels <<- labelsLocal[newIndices]
    
    print(table(labels))
    return()
  }
  
  if(!oversample){ return() }
...
```{r chunk_chapter5_task_5, message=FALSE, warning=FALSE, eval=FALSE, purl=FALSE}
if(!exists("done")){  # Do not calculate this more often than 1 time.
  train = c()
  labels = c()
  getData(oversample = FALSE, undersample = FALSE)
  done = 1
}
```
...
model %>%
  fit(x = train, y = to_categorical(matrix(labels, ncol = 1L), 5L),
...
Metrics::accuracy(pred_classes - 1L, labels)
...
pred_classes = apply(model %>% predict(train), 1, which.max)
print(table(pred_classes))
