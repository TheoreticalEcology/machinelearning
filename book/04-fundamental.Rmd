...
Now, we will come to artificial neural networks (ANNs), for which the topic of regularization is very important. We can specify the regularization in each layer via the kernel_regularization argument. 
...
```{r chunk_chapter4_18, results='hide', message=FALSE, warning=FALSE}
library(rpart)
library(rpart.plot)
...
```{r chunk_chapter4_21, message=FALSE, warning=FALSE}
...
```{r chunk_chapter4_23, cache=TRUE, results='hide', message=FALSE, warning=FALSE}
library(xgboost)
set.seed(123)
...
```{r chunk_chapter4_24__BRT1, cache=TRUE}
...
```{r chunk_chapter4_25__BRT2, cache=TRUE}
...
```{r chunk_chapter4_26__BRT3, cache=TRUE}
...
```{r chunk_chapter4_27__BRT4, cache=TRUE}
set.seed(123)
...
```{r chunk_chapter4_33, eval=FALSE, purl=FALSE}
x1 = seq(-3, 3, length.out = 100)
x2 = seq(-3, 3, length.out = 100)
x = expand.grid(x1, x2)
y = apply(X, 1, function(t) exp(-t[1]^2 - t[2]^2))
...
```{r chunk_chapter4_34, message=FALSE, warning=FALSE, echo=FALSE, purl=FALSE}
...
```{r chunk_chapter4_35, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
cat(
  '<iframe width="560" height="315" 
...
```{r chunk_chapter4_37, message=FALSE}
library(keras)
library(tensorflow)
library(tidyverse)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter4_60, eval=FALSE}
...
for(i in 1:3){ print(sum(data$cabin[data$pclass==i]=="") / sum(data$pclass==i)) }
...
# Impute values for "age":
  ...
  forest = rpart(age ~ ., data = tmp, control = rpart.control(minsplit = 15))
  rpart.plot::rpart.plot(forest)
  prediction = predict(forest, toPredict)
...
model %>%
  ...
  layer_dense(units = 50L, activation = "relu",
              bias_regularizer = regularizer_l1_l2(2),
              kernel_regularizer = regularizer_l1(.005)) %>%
  ...
  layer_dense(units = 20L, activation = "relu") %>%
  layer_dense(units = 10L, activation = "elu") %>%
  layer_dense(units = 2L, activation = "softmax")
...
```{r chunk_chapter4_task_43, eval=False, purl=FALSE}
prediction =
  model %>%
    predict(x = predict)
...
write.csv(data.frame(y = prediction), file = "submission.csv")
```
...
```{r chunk_chapter4_61, message=FALSE}
library(EcoData)
library(tidyverse)
library(mlr3)
...
```{r chunk_chapter4_66, eval=FALSE}
...
```{r chunk_chapter4_67__mlr2, eval=FALSE}
preds = 
...
```{r chunk_chapter4_71, eval=FALSE}
set.seed(123)
...
```{r chunk_chapter4_77, eval=FALSE}
set.seed(123)
...
After reading the above chapter about the mlr package, try to transfer it to the titanic data set.
Alternatively, you can also use other data sets from our challenge (e.g. the plant-pollinator data set, see the dataset chapter \@ref(datasets)).
...
```{r chunk_chapter4_task_44, message=FALSE, warning=FALSE}
