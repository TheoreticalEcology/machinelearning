...
Note:
The actions above operate on a local copy of the iris data set. You don't overwrite the base data and can use it again in a new R session.
...
```{r chunk_chapter2_16, error=TRUE, purl=FALSE}
...
```{r chunk_chapter2_17, eval=FALSE}
...
```{r chunk_chapter2_19, eval=TRUE, purl=FALSE}
iris$Sepal.Length[2] = "Hund"
iris$Sepal.Length = as.numeric(iris$Sepal.Length)
```
...
Taking the mean of a character sequence is not possible, so the result is NA (**N**ot **A**vailable, missing values).

But you can skip missing values with the option "na.rm = TRUE" of the "mean" function. To use it with the "apply" function, pass the argument.
Note: the wh

```{r chunk_chapter2_21, eval=TRUE}
...
```{r chunk_chapter2_26, eval=FALSE}
...
```{r chunk_chapter2_30, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
cat(
...
```{r chunk_chapter2_31, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
cat(
  '<iframe width="560" height="315"
...
```{r chunk_chapter2_32, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
cat(
  '<iframe width="560" height="315"
