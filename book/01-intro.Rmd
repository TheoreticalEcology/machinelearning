...
```{r chunk_chapter1_0, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
cat(
...
```{r chunk_chapter1_1__iris_plot, fig.width=10, fig.height=4}
colors = hcl.colors(3)
...
```{r chunk_chapter1_7, results='hide', message=FALSE, warning=FALSE}
library(dendextend)
```
...
```{r chunk_chapter1_16, results='hide', message=FALSE, warning=FALSE}
...
```{r chunk_chapter1_18, results='hide', message=FALSE, warning=FALSE}
mb = Mclust(traits)
```
...
```{r chunk_chapter1_25, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
cat(
  '<iframe width="560" height="315" 
...
```{r chunk_chapter1_26, results='hide', message=FALSE, warning=FALSE}
...
```{r chunk_chapter1_29, message=FALSE, warning=FALSE}
...
```{r chunk_chapter1_31, message=FALSE, warning=FALSE}
oldpar = par(mfrow = c(1, 2))
reprtree:::plot.getTree(m1, iris)
```
...
```{r chunk_chapter1_35, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
cat(
  '<iframe width="560" height="315" 
...
```{r chunk_chapter1_41, eval=FALSE}
...
```{r chunk_chapter1_44, eval=FALSE}
is.integer(5)
is.integer(5L)
...
```{r chunk_chapter1_52, eval=FALSE}
r_matrix = matrix(runif(10*10), 10,10)
...
```{r chunk_chapter1_55, cache=TRUE}
X = scale(iris[,1:4])
Y = iris[,5]
```
...
```{r chunk_chapter1_56, cache=TRUE}
...
```{r chunk_chapter1_57, cache=TRUE}
...
```{r chunk_chapter1_58, cache=TRUE}
model %>%
...
```{r chunk_chapter1_60, cache=TRUE}
model %>%
...
```{r chunk_chapter1_62, cache=TRUE}
library(tensorflow)
library(keras)
...
```{r chunk_chapter1_64, cache=TRUE}
plot(model_history)
```
...
```{r chunk_chapter1_66, cache=TRUE}
predictions = predict(model, X) # Probabilities for each class.
```
  
Get probabilities:

```{r chunk_chapter1_67, cache=TRUE}
...
```{r chunk_chapter1_68, cache=TRUE}
