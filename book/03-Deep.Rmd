...
```{r chunk_chapter3_1, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
cat(
  '<iframe width="560" height="315" 
...
```{r chunk_chapter3_2, message=FALSE}
library(EcoData)
data(nasa)
str(nasa)
...
```{r chunk_chapter3_24, eval=FALSE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	#Already sets R's random seed.
...
```{r chunk_chapter3_30, eval=FALSE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	#Already sets R's random seed.
...
model %>% 
	compile(loss = loss_categorical_crossentropy,
	        optimizer = keras::optimizer_rmsprop(lr = 0.0005))
