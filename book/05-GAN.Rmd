...
```{r chunk_chapter5_0, include=FALSE}
...
```{r chunk_chapter5_6__AEmnistoutput}
...
```{r chunk_chapter5_7__AEmnistfit}
library(tensorflow)
library(keras)
...
```{r chunk_chapter5_8__AEvisualization}
...
```{r chunk_chapter5_9__AEvisualizationContinuation}
...
```{r chunk_chapter5_11__AEmnistCNN}
down_size_model = keras_model_sequential()
down_size_model %>% 
...
```{r chunk_chapter5_21__VAEflower}
encoded = 10L
...
```{r chunk_chapter5_42, echo=FALSE, out.width="150%", out.height="150%"}
...
```{r chunk_chapter5_43, eval=FALSE}
library(keras)
library(tensorflow)
library(gym)
set_random_seed(321L, disable_gpu = FALSE)	#Already sets R's random seed.
