...
The noise (random vector with 100 values) is passed through the network and the output correspond to the number of pixels of one MNIST image (784). We therefore now define the discriminator function
...
```
And also the discriminator function
```{r}
...
### Flower - GAN
We can now also do the same for the flower dataset, we will write this now compleltly in our own following the steps also done for the MNIST data set.
...
## Autoencoder
An autoencoder is a type of artificial neural network used to learn efficient data encoding in an unsupervised manner. The aim of an autoencoder is to learn a representation (encoding) for a set of data, typically for dimensionality reduction, by training the network to ignore signal “noise”. Autoencoders consist of Encoder and a Decoder Networks. The encoder will compress the data into 2 dimensions and the decoder will reconstruct the original data
### Autoencoder - DNN MNIST
We will now write an autoencoder for the MNIST data set.

First we will split the data again
```{r}
library(keras)
library(tensorflow)
rotate = function(x) t(apply(x, 2, rev))
...
Then we will define a dense autoencoder and inputs will be compromized to two dimensions
```{r}
...
```
We will also need a network for the reconstruction of the images
```{r}
...
```
Now, we can combine the two models into one autoencoder and test it
```{r}
...
After training, we can now see how the network works
```{r}
...
First a split of the data again 
```{r,eval=FALSE}
data = tf$keras$datasets$mnist$load_data()
train = data[[1]]
...
The difference between a variational and a normal autoencoder is that the variational autoencoders tries to fit latent variables and their distributions which encode the images

To do so we will use the tensorflow probability, first need to split the data again 
```{r, eval=FALSE}
...
train_x = array(train[[1]]/255, c(dim(train[[1]])[1], 784L))
```
Then, we will use tensorflow probability to define priors (the a priori knowledge about the latent variables)
```{r}
...
```
Then, we define the two networks again 
```{r}
down_size_model = keras_model_sequential()
down_size_model %>% 
 layer_dense(units = 100L, input_shape = c(784L),activation = "relu") %>% 
...
up_size_model %>% 
 ...
 layer_dense(units = 784L, activation = "sigmoid")
```
We again combine the two networks into one abd define a optimizer
```{r}
batch_size = 32L
epochs = 10L
...
```
Now we can again define a batch function 
```{r}
get_batch = function(){
 indices = sample.int(nrow(train_x), batch_size)
