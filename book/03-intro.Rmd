...
```{r chunk_chapter3_64, cache=TRUE, message=FALSE, warning=FALSE}
plot(model_history)
```
...
We will now build a regressor for the airquality data set with Keras. We want to predict the variable "Ozone".
...
```{r chunk_chapter3_task_300, eval=FALSE, purl=FALSE}
fit = lm(Ozone ~ ., data = data)
pred_lm = predict(fit, data)
rmse_lm = mean(sqrt((y - pred_lm)^2))
rmse_keras = mean(sqrt((y - pred_keras)^2))
...
```{r chunk_chapter3_task_302, eval=TRUE}
