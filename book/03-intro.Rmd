...
## Small Introduction Into the Underlying Mathematical Concepts of all Following Lessons - Optional

If you want to know more about the underlying concepts of the following lessons, it is suggested to watch / view the following videos / sites:

<strong>This might be useful to understand the further concepts in more depth.</strong>

Consider the Links and videos with descriptions in parentheses as optional bonus.
...
```{r chunk_chapter3_45, eval=FALSE}
...
```{r chunk_chapter3_48, eval=FALSE}
is.integer(5)
is.integer(5L)
...
optimizer = tf$keras$optimizers$Adamax(lr = 0.05)
...
```{r chunk_chapter3_56, eval=FALSE}
r_matrix = matrix(runif(10*10), 10,10)
...
```{r chunk_chapter3_59, cache=TRUE}
X = scale(iris[,1:4])
Y = iris[,5]
```
...
```{r chunk_chapter3_61, cache=TRUE}
...
```{r chunk_chapter3_68, cache=TRUE, message=FALSE, warning=FALSE}
plot(model_history)
```
...
```{r chunk_chapter3_72, cache=TRUE}
...
model %>%
  compile(loss = loss_mean_squared_error, optimizer_adamax(0.05))
```
...
Because we do not want any compression or dilation, or other nonlinear effects, we use the simple linear layer (equal to no activation function at all). For more about activation functions, look for example <a href="https://en.wikipedia.org/wiki/Activation_function" target="_blank" rel="noopener">here</a>. Or wait for the next days.
...
<strong>Look at this slightly more complex model and compare the loss plot and the accuracy in contrast to the former.</strong>
...
<strong>Look at the little change in learning rates for the next 2 models and compare the loss plot and the accuracy in contrast to the former.</strong>
...
model %>%
  compile(loss = loss_mean_squared_error, optimizer_adamax(0.01))
