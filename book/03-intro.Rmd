...
```
```{r chunk_chapter3_25, message=FALSE, warning=FALSE}
library(dendextend)
...
```
```{r chunk_chapter3_27, message=FALSE, warning=FALSE}
library(dbscan)
...
```
```{r chunk_chapter3_28, message=FALSE, warning=FALSE}
library(mclust)
...
```{r chunk_chapter3_30, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
cat(
  '<iframe width="560" height="315" 
...
```{r chunk_chapter3_31, results='hide', message=FALSE, warning=FALSE}
...
```{r chunk_chapter3_34, message=FALSE, warning=FALSE}
...
```{r chunk_chapter3_36, message=FALSE, warning=FALSE}
oldpar = par(mfrow = c(1, 2))
reprtree:::plot.getTree(m1, iris)
```
...
```{r chunk_chapter3_69, cache=TRUE}
plot(model_history)
```
...
```{r chunk_chapter3_73, cache=TRUE}
