...
2. Try different learning rates and neural network sizes (increase/decrease number of hidden layers and neurons (units) in each layer). What happens?
...
```{=html}
  <details>
    ...
    <p>
```
```{r chunk_chapter3_task_31, eval=TRUE, include=TRUE}
data = airquality
```

**1. There are NAs in the data, which we have to remove because Keras and Torch cannot handle NAs!**

```{r chunk_chapter3_task_32, eval=TRUE, include=TRUE}
...
```{r chunk_chapter3_task_33, eval=TRUE, include=TRUE}
x = scale(data[,2:6])
y = data[,1]
```
...
```{r chunk_chapter3_task_34, eval=TRUE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter3_task_35, eval=TRUE, include=TRUE}
model %>%
  layer_dense(units = 20L, activation = "relu", input_shape = list(5L)) %>%
  layer_dense(units = 20L) %>%
  layer_dense(units = 20L) %>%
...
```{r chunk_chapter3_task_37, eval=TRUE, include=TRUE}
model_history =
  model %>%
...
```{r chunk_chapter3_task_38, eval=TRUE, message=FALSE, warning=FALSE, include=TRUE}
plot(model_history)
...
```{r chunk_chapter3_task_39, eval=TRUE, include=TRUE}
pred_keras = predict(model, x)
```

**9. Compare Keras model with a linear model.**

```{r chunk_chapter3_task_40, eval=TRUE, include=TRUE}
fit = lm(Ozone ~ ., data = data)
pred_lm = predict(fit, data)
rmse_lm = mean(sqrt((y - pred_lm)^2))
...
```{r chunk_chapter3_task_36_torch, eval=FALSE, purl=FALSE, include=TRUE}
library(torch)
...
```{r chunk_chapter3_task_37_torch, eval=FALSE, purl=FALSE, include=TRUE}
optimizer_torch = optim_adam(params = model_torch$parameters, lr = 0.001)
```
...
```{r chunk_chapter3_task_38_torch, eval=FALSE, purl=FALSE, include=TRUE}
# Calculate number of training steps.
epochs = 100
batch_size = 32
steps = round(nrow(x)/batch_size*epochs)
...
```{r chunk_chapter3_task_381_torch, eval=FALSE, purl=FALSE, include=TRUE}
plot(y = log_losses, x = 1:steps, xlab = "Epoch", ylab = "MSE")
```

**7. Create predictions.**

```{r chunk_chapter3_task_382_torch, eval=FALSE, purl=FALSE, include=TRUE}
...
```{r chunk_chapter3_task_39_torch, eval=FALSE, purl=FALSE, include=TRUE}
fit = lm(Ozone ~ ., data = data)
pred_lm = predict(fit, data)
rmse_lm = mean(sqrt((y - pred_lm)^2))
rmse_torch = mean(sqrt((y - pred_torch)^2))
...
```{r chunk_chapter3_task_41, eval=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter3_task_41_torch, eval=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
...
```{r chunk_chapter3_task_42, eval=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter3_task_43_torch, eval=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
...
```{r chunk_chapter3_task_43, eval=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter3_task_442_torch, eval=TRUE, message=FALSE, warning=FALSE, include=TRUE}
library(torch)
...
```{r chunk_chapter3_task_45, eval=TRUE, include=TRUE}
library(tensorflow)
library(keras)
set_random_seed(321L, disable_gpu = FALSE)	# Already sets R's random seed.
...
```{r chunk_chapter3_task_47_torch, eval=TRUE, include=TRUE}
library(torch)
