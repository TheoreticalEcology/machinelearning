...
```{r chunk_chapter3_61, eval=FALSE}
library(keras3)
# or library(torch)
```
...
```{r chunk_chapter3_62, eval=FALSE}
library(keras3)
library(tensorflow)
library(torch)
...
```{r chunk_chapter3_63, cache=TRUE, eval=FALSE}
X = scale(iris[,1:4])
Y = iris[,5]
```
...
```{r chunk_chapter3_64, cache=TRUE, eval=FALSE}
...
```{r chunk_chapter3_65, cache=TRUE, eval=FALSE}
...
```{r chunk_chapter3_66, cache=TRUE, eval=FALSE}
model |>
...
```{r chunk_chapter3_67, eval=FALSE}
model_torch = 
  nn_sequential(
    nn_linear(4L, 20L),
    nn_linear(20L, 20L),
...
```{r chunk_chapter3_68, cache=TRUE, eval=FALSE}
model |>
...
```{r chunk_chapter3_69, eval=FALSE}
...
```{r chunk_chapter3_70, cache=TRUE, eval=FALSE}
library(tensorflow)
library(keras3)
...
```{r chunk_chapter3_71, eval=FALSE}
library(torch)
...
```{r chunk_chapter3_72, cache=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
plot(model_history)
```

## Torch

```{r chunk_chapter3_73, eval=FALSE}
...
```{r chunk_chapter3_74, cache=TRUE, eval=FALSE}
predictions = predict(model, X) # Probabilities for each class.
```

Get probabilities:

```{r chunk_chapter3_75, cache=TRUE, eval=FALSE}
...
```{r chunk_chapter3_76, cache=TRUE, eval=FALSE}
...
```{r chunk_chapter3_77, cache=TRUE, eval=FALSE}
model_torch$eval()
...
```{r chunk_chapter3_78, cache=TRUE, eval=FALSE}
mean(preds == as.integer(iris$Species))
```

## Torch

```{r chunk_chapter3_78_torch, cache=TRUE, eval=FALSE}
...
```{r chunk_chapter3_79, cache=TRUE, eval=FALSE}
...
```{r chunk_chapter3_task_26, eval=FALSE}
library(tensorflow)
library(keras3)
...
```{r chunk_chapter3_task_26_torch, eval=FALSE}
library(torch)
