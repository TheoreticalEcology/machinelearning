...
In the next step we will fit a elastic-net on the training data of the inner split:

```{r }
library(glmnet)
library(glmnetUtils)
model = glmnet(survived~., family = "binomial", data = sub_train, alpha = 0.2)
```
...
```{r chunk_chapter4_60, warning=FALSE}
pred =
  model %>%
    predict(as.matrix(sub_test[,-1]), s = 0.5, type = "response")
...
When we are satisfied with the performance of our model, we will create predictions for the new observations on the submission server. To do so, we take all observations that belong to new observations (use the filter function) and remove the survived (NAs) columns:
...
```{r chunk_chapter4_63, warning=FALSE}
pred = model %>% 
  predict(as.matrix(submit), s = 0.1, type = "response")
```
...
We used only one split (the split for the submission server doesn't count) to evaluate the performance of the model before we made the final predictions. To tune $\lambda$, we would need to split the data another time (called nested CV).
...
The "double CV" approach is called nested CV. Luckily, the glmnet package has already a function called cv.glmnet which does a CV for us on the data we provide:

```{r}
m1 = cv.glmnet(survived~., data = sub_train, nfolds = 10, family = "binomial", alpha = 0.2)
...
pred =
  m1 %>%
    predict(as.matrix(sub_test[,-1]), s = lambda, type = "response")
...
::: {.callout-caution icon="false"}
#### Exercise: Tuning $\alpha$

Let's implement manually a nested CV to tune the $\alpha$. Let's start with a 3CVx3CV and 10x different mtry values:

```{r}
data_obs = data_sub[!is.na(data_sub$survived),] 
cv = 5
cv_inner = 5
hyper_alpha = seq(0, 1, length.out = 20)
...
1.  Fit glmnet on the obs_data with the alpha hyperparameter that has shown the highest AUC

2.  Fit for each alpha which we have found a glmnet and combine (average) the predictions
...
alpha = results[which.max(results$AUC),]$alpha

model = glmnet(survived~., data = data_obs, alpha = alpha, family = "binomial")
predictions = predict(model, data_new, s = 0.01, type = "response")
...
prediction_ensemble = 
  sapply(1:nrow(results), function(i) {
    ...
    model = glmnet(survived~., data = data_obs, alpha = alpha)
    return(predict(model, data_new, s = 0.01, type = "response"))
  })
