...
```{r chunk_chapter3_46, eval=FALSE}
...
```{r chunk_chapter3_47, eval=FALSE}
...
```{r chunk_chapter3_50, , eval=FALSE}
r_matrix = matrix(runif(10*10), 10, 10)
...
```{r chunk_chapter3_51, eval=FALSE}
r_matrix = matrix(runif(10*10), 10, 10)
...
```{r chunk_chapter3_task_6, eval=FALSE}
x = 100:1
y = as.double(100:1)
...
```{r chunk_chapter3_task_7, eval=FALSE, include=TRUE}
library(tensorflow)
library(keras)
...
```{r chunk_chapter3_task_12, eval=FALSE, include=TRUE}
do_something_TF = function(x = matrix(0.0, 10L, 10L)){
...
```{r chunk_chapter3_task_13, eval=FALSE, include=TRUE}
test = matrix(0.0, 100L, 100L)
microbenchmark::microbenchmark(do_something_R(test), do_something_TF(test))
...
```{r chunk_chapter3_task_14, eval=FALSE, include=TRUE}
test = matrix(0.0, 1000L, 500L)
testTF = tf$constant(test)
...
```{r chunk_chapter3_task_15, eval=FALSE}
...
```{r chunk_chapter3_task_16, eval=FALSE, include=TRUE}
library(tensorflow)
library(keras3)
...
```{r chunk_chapter3_task_19, eval=FALSE}
...
```{r chunk_chapter3_task_20, eval=FALSE}
(tape$gradient(y, x))
```

We can also calculate the second order derivative $f''(x) = 10$:

```{r chunk_chapter3_task_21, eval=FALSE}
...
```{r chunk_chapter3_task_22, eval=FALSE}
...
```{r chunk_chapter3_task_23, eval=FALSE}
summary(lm(y~x))
```
...
```{r chunk_chapter3_task_24, eval=FALSE}
library(tensorflow)
library(keras3)
...
```{r chunk_chapter3_task_25, eval=FALSE, include=TRUE, warning=FALSE}
library(tensorflow)
library(keras3)
...
```{r chunk_chapter3_53, eval=FALSE}
library(torch)
```
...
```{r chunk_chapter3_54, eval=FALSE}
a = torch_tensor(1.)
```
...
```{r chunk_chapter3_55, eval=FALSE}
...
```{r chunk_chapter3_57, eval=FALSE}
a = torch_tensor(5.)
b = torch_tensor(10.)
print(a+b)
print(a/b)
...
```{r chunk_chapter3_58, eval=FALSE}
...
```{r chunk_chapter3_59, eval=FALSE}
class(d)
class(as.numeric(d))
```
...
```{r chunk_chapter3_task_torch_6, eval=FALSE}
x = 100:1
y = as.double(100:1)
...
```{r chunk_chapter3_task_torch_7, eval=FALSE, include=TRUE}
library(torch)
...
```{r chunk_chapter3_task_torch_8, eval=FALSE}
do_something_R = function(x = matrix(0.0, 10L, 10L)){
  mean_per_row = apply(x, 1, mean)
  result = x - mean_per_row
  return(result)
...
```{r chunk_chapter3_task_torch_12, eval=FALSE, include=TRUE}
do_something_torch = function(x = matrix(0.0, 10L, 10L)){
...
```{r chunk_chapter3_task_torch_13, eval=FALSE, include=TRUE}
test = matrix(0.0, 100L, 100L)
microbenchmark::microbenchmark(do_something_R(test), do_something_torch(test))
...
```{r chunk_chapter3_task_torch_15, eval=FALSE}
A = matrix(c(1, 2, 0, 0, 2, 0, 2, 5, 3), 3, 3)
...
```{r chunk_chapter3_task_torch_16, eval=FALSE, include=TRUE}
library(torch)
...
```{r chunk_chapter3_task_torch_17, eval=FALSE}
f = function(x){ return(5.0 * torch_pow(x, 2.) + 3.0) }
```

We want to calculate the derivative for $x = 2.0$:

```{r chunk_chapter3_task_torch_18, eval=FALSE}
...
```{r chunk_chapter3_task_torch_19, eval=FALSE}
y = f(x)
...
```{r chunk_chapter3_task_torch_20, eval=FALSE}
x$grad
```

We can also calculate the second order derivative $f''(x) = 10$:

```{r chunk_chapter3_task_torch_21, eval=FALSE}
x = torch_tensor(2.0, requires_grad = TRUE)
y = f(x)
...
```{r chunk_chapter3_task_torch_22, eval=FALSE}
...
```{r chunk_chapter3_task_torch_23, eval=FALSE}
summary(lm(y~x))
```
...
```{r chunk_chapter3_task_torch_24, eval=FALSE}
library(torch)
...
```{r chunk_chapter3_task_torch_25, eval=FALSE, include=TRUE}
library(torch)
torch::torch_manual_seed(42L)
